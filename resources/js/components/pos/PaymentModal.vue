        ...
        <div v-if="isPaymentModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/85 backdrop-blur-sm transition-opacity">
            <div class="bg-white dark:bg-zinc-900 w-full max-w-xl rounded-3xl border border-slate-100 dark:border-zinc-800 overflow-hidden shadow-2xl flex flex-col">
                ...
                <div class="p-5 border-b border-slate-100 dark:border-zinc-800 flex justify-between items-center bg-slate-50/50 dark:bg-zinc-900/40">
                    ...
                    <button @click="emit('closePaymentModal')" class="p-1.5 rounded-full text-slate-400 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors">
                        <X class="h-4 w-4" />
                    </button>
                </div>
                ...
                <div class="p-6 space-y-5 overflow-y-auto max-h-[75vh] custom-scrollbar">
                    ...
                    <div v-if="paymentMethod === 'cash'" class="p-4 bg-slate-50 dark:bg-zinc-800/50 rounded-2xl space-y-3 border border-slate-200/60 dark:border-zinc-700">
                        <div class="grid grid-cols-4 gap-2 text-xs">
                            <button @click="emit('update:amountPaidInput', finalTotal)" type="button" class="py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-xl font-bold text-slate-700 dark:text-zinc-200">Uang Pas</button>
                            <button @click="emit('update:amountPaidInput', 25000)" type="button" class="py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-xl font-bold text-slate-700 dark:text-zinc-200">25k</button>
                            <button @click="emit('update:amountPaidInput', 50000)" type="button" class="py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-xl font-bold text-slate-700 dark:text-zinc-200">50k</button>
                            <button @click="emit('update:amountPaidInput', 100000)" type="button" class="py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-xl font-bold text-slate-700 dark:text-zinc-200">100k</button>
                        </div>
                ...
                <div class="p-5 border-t border-slate-100 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-900/50 grid grid-cols-2 gap-3">
                    <button @click="emit('closePaymentModal')" type="button" class="py-3 bg-white border border-slate-200 text-slate-700 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-300 font-bold rounded-2xl text-xs">
                        Batal
                    </button>
                    ...
                    <button 
                        ...
                        type="button"
                        class="py-3 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl text-xs shadow-md flex items-center justify-center gap-2 disabled:opacity-50"
                    >
                    ...
                    <button 
                        ...
                        type="button"
                        class="py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl text-xs shadow-md"
                    >
                        Bayar Tunai
                    </button>
                </div>
        ...
        <div v-if="isQrisModalOpen" class="fixed inset-0 z-100 flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md">
            <div class="bg-white dark:bg-zinc-900 w-full max-w-2xl rounded-4xl border border-slate-200 dark:border-zinc-800 shadow-2xl overflow-hidden flex flex-col">
                <div class="bg-slate-50 dark:bg-zinc-800 p-5 px-8 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between">
                    ...
                    <button @click="emit('closeQrisModal')" class="p-2 bg-white dark:bg-zinc-700 rounded-full text-slate-400 border border-slate-200 dark:border-zinc-600">
                        <X class="w-5 h-5" />
                    </button>
                </div>
        ...
        <div v-if="isSuccessModalOpen" class="fixed inset-0 z-100 flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md">
            <div class="bg-white dark:bg-zinc-900 w-full max-w-xl rounded-4xl border border-slate-200 dark:border-zinc-800 shadow-2xl overflow-hidden p-8 text-center space-y-6">
                ...
                <div class="flex gap-3 pt-4">
                    <button @click="emit('handlePrintReceipt')" class="flex-1 py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-black rounded-2xl text-sm shadow-lg">
                        🖨️ Cetak Struk
                    </button>
                    <button @click="emit('closeSuccessModal')" class="flex-1 py-3.5 bg-white border border-slate-200 text-slate-700 font-black rounded-2xl text-sm">
                        Selesai
                    </button>
                </div>
            </div>
